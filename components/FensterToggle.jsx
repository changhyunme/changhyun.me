...
export default function FensterToggle() {
  const { breitbild, toggleBreitbild, lightmode, toggleLightmode, language, setLanguage } = useStore();
  ...
  return (
    <div className=" flex flex-row items-center ml-auto text-white">
      ...
      <div className="relative">
        ...
        {showThemeTooltip && (
          <div className={`w-31 bg-ui/70 text-text/70 ${TooltipClass} ${breitbild ? "-bottom-9" : "-top-9"}`}>
            {lightmode ? t('tooltip.disableLightMode') : t('tooltip.disableDarkMode')}
          </div>
        )}
      </div>
      ...
      <div className="relative">
        ...
        {showLanguageMenu && (
          <div className={`absolute ${breitbild ? 'bottom-8' : 'top-8'} right-0
                          bg-bgSub border border-border/40 rounded-md shadow-lg
                          min-w-36 py-1 z-50 backdrop-blur-sm`}
            ...
            {['en', 'ko', 'jp', 'de'].map((lang) => (
              <button
                ...
                className={`w-full px-3 py-2 text-left text-sm flex items-center gap-2
                           ...
                           ${language === lang ? 'bg-primary/10 text-primary font-semibold' : 'text-text'}`}
              >
      ...
      <div className="relative">
        ...
        {showRatioTooltip && (
          <div className={`w-29 bg-ui/70 text-text/70 ${TooltipClass}`}>
            {breitbild ? t('tooltip.condensedView') : t('tooltip.expandedView')}
          </div>
        )}
      </div>
